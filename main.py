...
Automatiza o processamento de áudio de vídeos:
 ...
 13. Auto-update: verifica, baixa o instalador e atualiza automaticamente
...
URL_VERSAO    = f"https://raw.githubusercontent.com/{GITHUB_REPO}/main/version.txt"
...
class SplashScreen(tk.Tk):
    ...
    def _build(self):
        ...
        self.canvas.create_text(
            ...
            text=f"v{VERSAO_ATUAL}  ·  Python + ffmpeg  ·  © 2025 AudioFix",
            font=("Segoe UI", 8),
            fill=_MUTED, anchor="center"
        )
...
def _launch_main():
    app = AudioFixApp()
    # Verifica atualizações 3 segundos após o app estar visível
    app.after(3000, lambda: check_for_updates_async(app))
    app.mainloop()
...
def _get_platform_asset_priority():
    """Retorna extensões preferidas por plataforma, em ordem de preferência."""
    s = platform.system()
    if s == "Windows":
        return [".exe", ".msi", ".zip"]
    elif s == "Darwin":
        ...
        return [".AppImage", ".deb", ".tar.gz", ".zip"]


def _fetch_latest_release():
    """
    Consulta a GitHub Releases API e retorna:
        (tag_str, download_url, asset_name, release_notes)
    ou lança exceção em caso de falha.
    """
    headers = {"Accept": "application/vnd.github.v3+json",
               "User-Agent": f"AudioFixPro/{VERSAO_ATUAL}"}
    resp = requests.get(URL_API, timeout=10, headers=headers)
    resp.raise_for_status()
    data = resp.json()

    tag   = data.get("tag_name", "").lstrip("vV").strip()
    notes = (data.get("body") or "").strip()[:800]  # limita notas a 800 chars
    assets = data.get("assets", [])

    priority_exts = _get_platform_asset_priority()
    chosen_url  = None
    chosen_name = None

    # Tenta casar pela extensão mais preferida primeiro
    for ext in priority_exts:
        for asset in assets:
            name = asset.get("name", "")
            if name.lower().endswith(ext):
                chosen_url  = asset.get("browser_download_url")
                chosen_name = name
                break
        if chosen_url:
            break

    # Fallback: primeiro asset disponível
    if not chosen_url and assets:
        chosen_url  = assets[0].get("browser_download_url")
        chosen_name = assets[0].get("name", "AudioFixPro_Setup.exe")

    # Último fallback: abre página de releases no navegador
    if not chosen_url:
        chosen_url  = data.get("html_url", URL_DOWNLOAD)
        chosen_name = None   # None = sinaliza que não há download direto

    return tag, chosen_url, chosen_name, notes


def check_for_updates_async(parent):
    """
    Dispara a checagem em background.
    Se encontrar nova versão, abre UpdateDialog na thread principal.
    Erros de rede/API são silenciados (não incomodam o usuário).
    """
    def _worker():
        try:
            tag, url, name, notes = _fetch_latest_release()
            if tag and tag != VERSAO_ATUAL:
                parent.after(0, lambda: UpdateDialog(parent, tag, url, name, notes))
        except Exception as e:
            print(f"[AutoUpdate] Falha na verificação: {e}")
...
class UpdateDialog(tk.Toplevel):
    """
    Janela de atualização com três fases:
        FASE 1 — Notificação: mostra versão nova, notas e botão de download
        FASE 2 — Download:    barra de progresso + velocidade em tempo real
        FASE 3 — Concluído:   lança o instalador e fecha o aplicativo
    """

    _PHASE_NOTIFY   = "notify"
    _PHASE_DOWNLOAD = "download"
    _PHASE_DONE     = "done"
    _PHASE_ERROR    = "error"

    def __init__(self, parent, latest_version, download_url,
                 asset_name=None, release_notes=""):
        super().__init__(parent)
        self.title("Atualização disponível — AudioFix Pro")
        ...
        self._notes         = release_notes
        self._dest_path     = None
        self._phase         = self._PHASE_NOTIFY
    ...
    def _build(self):
        ...
        hdr.pack(fill="x")
        tk.Label(hdr, text="🚀", font=("Segoe UI Emoji", 26), bg=PANEL).pack()
        tk.Label(hdr, text="Nova versão disponível!",
                 font=("Segoe UI", 15, "bold"), bg=PANEL, fg=TEXT).pack(pady=(2, 0))
        self._subtitle = tk.Label(
            hdr,
            text=f"AudioFix Pro  v{VERSAO_ATUAL}  →  v{self._latest}",
            font=("Segoe UI", 10), bg=PANEL, fg=ACC2
        )
        self._subtitle.pack()

        # Corpo rolável
        body = tk.Frame(self, bg=DARK, padx=22, pady=14)
        ...
        if self._notes:
            tk.Label(body, text="📋  O que há de novo:",
                     font=FONT_BOLD, bg=DARK, fg=MUTED).grid(row=0, column=0, sticky="w", pady=(0, 4))
            notes_card = tk.Frame(body, bg=CARD, padx=12, pady=10)
            notes_card.grid(row=1, column=0, sticky="ew", pady=(0, 12))
            notes_card.columnconfigure(0, weight=1)
            notes_txt = tk.Text(
                notes_card, height=5, font=FONT_MONO,
                bg="#111116", fg="#c8c8e0",
                relief="flat", state="normal", wrap="word",
                bd=0, padx=6, pady=4
            )
            notes_txt.insert("1.0", self._notes)
            notes_txt.config(state="disabled")
            notes_txt.grid(row=0, column=0, sticky="ew")
            sb_n = ttk.Scrollbar(notes_card, command=notes_txt.yview)
            sb_n.grid(row=0, column=1, sticky="ns")
            notes_txt.config(yscrollcommand=sb_n.set)

        # ── Área de progresso (visível só no download) ─────────────────────
        self._prog_card = tk.Frame(body, bg=CARD, padx=14, pady=12)
        self._prog_card.grid(row=2, column=0, sticky="ew", pady=(0, 10))
        self._prog_card.columnconfigure(0, weight=1)

        self._prog_bar = ttk.Progressbar(
            self._prog_card, mode="determinate",
            maximum=100, length=400
        )
        self._prog_bar.grid(row=0, column=0, sticky="ew")

        self._prog_lbl = tk.Label(
            self._prog_card, text="Aguardando…",
            font=FONT_STATUS, bg=CARD, fg=MUTED
        )
        self._prog_lbl.grid(row=1, column=0, sticky="w", pady=(4, 0))

        self._speed_lbl = tk.Label(
            self._prog_card, text="",
            font=FONT_SMALL, bg=CARD, fg=MUTED
        )
        self._speed_lbl.grid(row=2, column=0, sticky="w")

        self._eta_lbl = tk.Label(
            self._prog_card, text="",
            font=FONT_SMALL, bg=CARD, fg=MUTED
        )
        self._eta_lbl.grid(row=3, column=0, sticky="w")

        # Começa oculto — exibido ao iniciar o download
        self._prog_card.grid_remove()

        # ── Aviso sobre o instalador ───────────────────────────────────────
        warn_card = tk.Frame(body, bg="#1a1500", padx=12, pady=8)
        warn_card.grid(row=3, column=0, sticky="ew", pady=(0, 6))
        tk.Label(
            warn_card,
            text="⚠️  O AudioFix Pro será fechado automaticamente após o download\n"
                 "    para que o instalador possa atualizar os arquivos.",
            font=FONT_SMALL, bg="#1a1500", fg=WARN,
            justify="left", wraplength=480
        ).pack(anchor="w")

        # ── Rodapé com botões ──────────────────────────────────────────────
        footer = tk.Frame(self, bg=DARK, padx=22, pady=12)
        footer.pack(fill="x")

        self._cancel_btn = tk.Button(
            footer, text="Agora não",
            font=FONT_LABEL, bg=CARD, fg=MUTED,
        ...
        self._open_btn = tk.Button(
            footer, text="🌐  Ver no GitHub",
            font=FONT_LABEL, bg=CARD, fg=MUTED,
            relief="flat", padx=14, pady=9, cursor="hand2",
            command=lambda: webbrowser.open(URL_DOWNLOAD)
        )
        self._open_btn.pack(side="left", padx=(6, 0))

        # Botão principal muda de aparência conforme a fase
        self._action_btn = tk.Button(
            footer,
            text="⬇  Baixar e Instalar Agora",
        ...
        # Se não há URL de download direto, muda para abrir navegador
    ...
    def _on_action(self):
        if self._phase == self._PHASE_NOTIFY:
            self._start_download()
        elif self._phase == self._PHASE_ERROR:
            # Permite tentar novamente
            self._phase = self._PHASE_NOTIFY
            self._start_download()

    def _start_download(self):
        if not self._asset_name or not self._dl_url:
            webbrowser.open(URL_DOWNLOAD)
            return

        self._phase = self._PHASE_DOWNLOAD
        self._cancelled = False

        # Atualiza UI
        self._prog_card.grid()          # exibe card de progresso
        self._action_btn.config(
            state="disabled",
            text="⏳  Baixando…",
            bg=MUTED
        ...
        self._prog_lbl.config(text="Conectando ao servidor…", fg=TEXT)
        self._prog_bar.config(value=0, mode="indeterminate")
    ...
    def _download_worker(self):
        """Thread de download — atualiza a UI via after()."""
        try:
            dest = os.path.join(
                tempfile.gettempdir(),
                f"AudioFixPro_Update_{self._latest}_{self._asset_name}"
            )

            # Suporte a retomada parcial (Range request)
            resume_from  = 0
            extra_headers = {}
            ...
            resp = requests.get(
                self._dl_url, stream=True,
                timeout=30, headers=headers
            )
            resp.raise_for_status()

            content_length = int(resp.headers.get("content-length", 0))
            self._bytes_total = content_length + resume_from
            self._bytes_dl    = resume_from

            # Muda barra para modo determinado assim que souber o tamanho
            if content_length > 0:
                self.after(0, lambda: self._prog_bar.config(
    ...
    def _tick_progress(self):
        """Atualiza barra, velocidade e ETA — chamado pela thread principal."""
        if self._bytes_total > 0:
            pct = min((self._bytes_dl / self._bytes_total) * 100, 100)
            self._prog_bar.config(value=pct)
        else:
            pct = 0

        elapsed = max(time.time() - self._start_time, 0.001)
        speed   = self._bytes_dl / elapsed          # bytes/s
        ...
        dl_str   = fmt_bytes(self._bytes_dl)
        tot_str  = fmt_bytes(self._bytes_total) if self._bytes_total else "?"
        spd_str  = f"{fmt_bytes(int(speed))}/s"

        if speed > 0 and self._bytes_total > 0:
            eta_s = (self._bytes_total - self._bytes_dl) / speed
            eta_m = int(eta_s // 60)
            eta_s = int(eta_s % 60)
            eta_str = f"Tempo restante: {eta_m:02d}:{eta_s:02d}"
        else:
            eta_str = ""

        self._prog_lbl.config(
            text=f"Baixando…  {int(pct)}%  ({dl_str} / {tot_str})",
            fg=TEXT
        )
        self._speed_lbl.config(text=f"Velocidade: {spd_str}")
    ...
    def _on_download_complete(self):
        ...
        self._prog_lbl.config(
            text="✅  Download concluído! Iniciando instalador…",
            fg=GREEN
        )
        ...
        # Pequena pausa para o usuário ver a mensagem antes de fechar
        self.after(1200, self._launch_installer_and_exit)

    def _on_download_error(self, error_msg):
        self._phase = self._PHASE_ERROR
        self._prog_bar.stop()
        self._prog_bar.config(mode="determinate", value=0)
        self._prog_lbl.config(text=f"❌  {error_msg}", fg=RED)
        self._speed_lbl.config(text="")
        self._eta_lbl.config(text="")
        self._action_btn.config(
            state="normal",
    ...
    def _launch_installer_and_exit(self):
        """Lança o instalador baixado e encerra o aplicativo atual."""
        if not self._dest_path or not os.path.isfile(self._dest_path):
            messagebox.showerror(
                "Arquivo não encontrado",
                f"O arquivo de instalação não foi localizado:\n{self._dest_path}"
            )
            return
        ...
        try:
            ...
            if sys_name == "Windows":
                # /S = instalação silenciosa (NSIS); sem /S = wizard interativo
                # Usamos wizard interativo para o usuário acompanhar
                os.startfile(self._dest_path)

            elif sys_name == "Darwin":
                subprocess.Popen(["open", self._dest_path])

            else:
                # Linux: torna executável e roda
                name = self._dest_path
                if name.lower().endswith(".appimage"):
                    os.chmod(self._dest_path, 0o755)
                    subprocess.Popen([self._dest_path])
                elif name.lower().endswith(".deb"):
                    subprocess.Popen(
                        ["pkexec", "dpkg", "-i", self._dest_path]
                    )
                else:
                    subprocess.Popen(["xdg-open", self._dest_path])
        ...
        # ── Fecha o aplicativo para liberar arquivos em uso ────────────────
        self.after(600, self._force_exit)
    ...
    def _on_close(self):
        """Fecha o diálogo sem fazer nada (download ainda não iniciado)."""
        if self._phase == self._PHASE_DOWNLOAD:
