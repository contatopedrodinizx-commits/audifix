#!/usr/bin/env python3
"""
AudioFix Pro v5.6 + InvisibleMark v4.1
Integração: aba "🛡 InvisibleMark" adicionada ao AudioFix Pro.
Proteção adversarial imperceptível de faces em vídeo.
"""
...
def _check_opencv_silent():   return _is_importable("cv2")
...
# ─── Paleta InvisibleMark (mapeada para AudioFix) ─────────────────────────────
IM_HIGHLIGHT  = "#4d9eff"   # azul destaque
IM_GREEN_OK   = "#2ea84a"   # verde status
IM_AMBER      = "#d97706"   # alerta
IM_BTN_GREEN  = "#1f7a30"
IM_BTN_BLUE   = "#1a5fc7"
IM_BTN_HOVER  = "#2e7af0"
IM_BTN_RED    = "#c02f2c"
...
# ═══════════════════════════════════════════════════════════════════════════════
#  ██ INVISIBLEMARK — Núcleo de processamento ██
# ═══════════════════════════════════════════════════════════════════════════════

_IM_NOISE_CACHE = {}
_IM_NUM_CORES   = max(1, multiprocessing.cpu_count() - 1)
_IM_LAST_FACES  = []
_IM_FACE_CACHE_FRAMES = 3


def _im_get_cached_noise(shape_key, strength, seed=0):
    import numpy as np
    key = (shape_key, strength, seed % 16)
    if key not in _IM_NOISE_CACHE:
        rng = np.random.RandomState(seed % 16)
        h, w = shape_key
        noise = rng.randint(-strength, strength + 1, (h, w, 3), dtype=np.int8)
        _IM_NOISE_CACHE[key] = noise
        if len(_IM_NOISE_CACHE) > 64:
            oldest = next(iter(_IM_NOISE_CACHE))
            del _IM_NOISE_CACHE[oldest]
    return _IM_NOISE_CACHE[key]


def _im_get_face_detector():
    import cv2
    cascade_path = cv2.data.haarcascades + "haarcascade_frontalface_default.xml"
    return cv2.CascadeClassifier(cascade_path)


def _im_detect_faces(frame, detector, scale=1.1, neighbors=4, min_size=40,
                     use_cache=True, cache_counter=0):
    global _IM_LAST_FACES
    import cv2
    import numpy as np

    if use_cache and cache_counter % _IM_FACE_CACHE_FRAMES != 0:
        return _IM_LAST_FACES

    h, w = frame.shape[:2]
    scale_factor = 0.5 if max(h, w) > 720 else 1.0
    small = cv2.resize(frame, (int(w * scale_factor), int(h * scale_factor)),
                       interpolation=cv2.INTER_NEAREST) if scale_factor < 1.0 else frame
    ...
    faces_small = detector.detectMultiScale(
        ...
        minSize=(int(min_size * scale_factor), int(min_size * scale_factor)),
        flags=cv2.CASCADE_SCALE_IMAGE)

    if len(faces_small) > 0 and scale_factor < 1.0:
        inv = 1.0 / scale_factor
        faces = np.array([[int(x*inv), int(y*inv), int(w*inv), int(h*inv)]
                          for (x, y, w, h) in faces_small])
    elif len(faces_small) > 0:
        faces = faces_small
    else:
        faces = []

    _IM_LAST_FACES = faces
    return faces


def _im_adversarial_noise(region, strength=6, frame_idx=0):
    import numpy as np
    import cv2
    h, w = region.shape[:2]
    effective_strength = min(strength, 5)
    noise = _im_get_cached_noise((h, w), effective_strength, frame_idx).astype(np.int16)
    lum = cv2.cvtColor(region, cv2.COLOR_BGR2GRAY).astype(np.float32) / 255.0
    mask = 1.0 - np.abs(lum - 0.5) * 1.5
    mask = np.clip(mask, 0.1, 1.0)[:, :, np.newaxis]
    noise_scaled = (noise * mask).astype(np.int16)
    result = np.clip(region.astype(np.int16) + noise_scaled, 0, 255)
    return result.astype(np.uint8)


def _im_dct_attack(region, strength=5):
    import numpy as np
    import cv2
    h, w = region.shape[:2]
    h8 = (h // 8) * 8
    ...
    if h8 < 8 or w8 < 8:
        return region
    result = region.copy()
    roi = result[:h8, :w8].astype(np.float32)
    for c in range(3):
        ch = roi[:, :, c]
        dct = cv2.dct(ch)
        perturb_range = strength * 0.5
        rng = np.random.RandomState(c + int(h) + int(w))
        dct[2:8, 2:8] += rng.uniform(-perturb_range, perturb_range, (6, 6)).astype(np.float32)
        reconstructed = np.clip(cv2.idct(dct), 0, 255)
        roi[:, :, c] = reconstructed
    result[:h8, :w8] = roi.astype(np.uint8)
    return result


def _im_color_micro_shift(region, hue_shift=2, strength=5):
    import numpy as np
    import cv2
    if hue_shift > 3:
        hue_shift = min(3, hue_shift // 2)
    hsv = cv2.cvtColor(region, cv2.COLOR_BGR2HSV).astype(np.float32)
    hsv[:, :, 0] = (hsv[:, :, 0] + hue_shift) % 180
    sat_scale = 1.0 - (strength * 0.003)
    hsv[:, :, 1] = np.clip(hsv[:, :, 1] * sat_scale, 0, 255)
    return cv2.cvtColor(hsv.astype(np.uint8), cv2.COLOR_HSV2BGR)


def _im_landmark_perturbation(region, strength=4):
    import numpy as np
    import cv2
    h, w = region.shape[:2]
    result = region.copy().astype(np.int16)
    importance_map = np.zeros((h, w), dtype=np.float32)
    zones = [
        (0.20, 0.22, 0.38, 0.48, 1.2),
        (0.62, 0.22, 0.80, 0.48, 1.2),
        (0.35, 0.45, 0.65, 0.68, 0.8),
        (0.22, 0.65, 0.78, 0.88, 1.0),
    ]
    for (rx1, ry1, rx2, ry2, weight) in zones:
    ...
    ksize = max(3, (min(h, w) // 8) | 1)
    importance_map = cv2.GaussianBlur(importance_map, (ksize, ksize), 0)
    effective_strength = min(strength, 4)
    rng = np.random.RandomState(h + w)
    noise = rng.randint(-effective_strength, effective_strength + 1, (h, w, 3), dtype=np.int16)
    noise_weighted = (noise * importance_map[:, :, np.newaxis]).astype(np.int16)
    return np.clip(result + noise_weighted, 0, 255).astype(np.uint8)


def _im_temporal_blend(frame, prev_frame, alpha=0.04):
    import cv2
...
def _im_pixel_shuffle(region, strength):
    import numpy as np
    result = region.copy()
    h, w = region.shape[:2]
    block = 2
    prob = min(0.12, strength * 0.015)
    rng = np.random.RandomState(h + w + strength)
    for y in range(0, h - block, block):
        for x in range(0, w - block, block):
            if rng.random() < prob:
                b = result[y:y+block, x:x+block].copy()
                flat = b.reshape(-1, 3)
                result[y:y+block, x:x+block] = flat[::-1].reshape(b.shape)
    return result


def _im_apply_pipeline(frame, prev_frame, faces, settings, frame_idx=0):
    import numpy as np
    mode      = settings.get("mode", "faces_only")
    strength  = settings.get("strength", 6)
    use_noise = settings.get("adv_noise", True)
    use_dct   = settings.get("freq_disruption", True)
    use_temp  = settings.get("temporal", True)
    use_color = settings.get("color_shift", True)
    use_lm    = settings.get("landmark_noise", True)
    use_pixel = settings.get("pixel_shuffle", False)

    result = frame

    if use_temp and prev_frame is not None:
        result = _im_temporal_blend(result, prev_frame, alpha=0.04)
    ...
    if mode == "full_frame":
        ...
        if len(faces) > 0:
            fh, fw = frame.shape[:2]
            padding = max(8, strength)
            for (x, y, w, h) in faces:
                regions.append((
                    max(0, x - padding), max(0, y - padding),
                    min(fw, x + w + padding), min(fh, y + h + padding)
                ))
    ...
    for (x1, y1, x2, y2) in regions:
        ...
        if use_noise:
            region = _im_adversarial_noise(region, strength, frame_idx)
        if use_lm and rw > 32 and rh > 32:
            region = _im_landmark_perturbation(region, min(strength, 4))
        if use_dct:
            region = _im_dct_attack(region, strength)
        if use_color:
            region = _im_color_micro_shift(region, hue_shift=min(2, strength // 3))
        if use_pixel:
            region = _im_pixel_shuffle(region, strength)
        result[y1:y2, x1:x2] = region

    return result


def _im_video_has_audio(video_path):
    try:
...
def _im_merge_audio(original_video, processed_video, final_output):
    import shutil
    cmd = [
        "ffmpeg", "-y",
        "-i", processed_video,
    ...
    if result.returncode != 0:
        raise RuntimeError(f"ffmpeg falhou ao mesclar áudio:\n{result.stderr}")
...
class InvisibleMarkTab(tk.Frame):
    """
    Aba completa do InvisibleMark integrada ao AudioFix Pro.
    Preserva toda a lógica original do InvisibleMark v4.1.
    """
    ...
    def __init__(self, parent, **kwargs):
        ...
        self.protect_mode = tk.StringVar(value="faces_only")

        self.use_adv_noise = tk.BooleanVar(value=True)
        self.use_freq      = tk.BooleanVar(value=True)
        self.use_temporal  = tk.BooleanVar(value=True)
        self.use_color     = tk.BooleanVar(value=True)
        self.use_landmark  = tk.BooleanVar(value=True)
        self.use_pixel     = tk.BooleanVar(value=False)
        self.keep_audio    = tk.BooleanVar(value=True)

        self.strength_val  = tk.IntVar(value=6)
        self.frame_skip    = tk.IntVar(value=2)
        self.face_cache_n  = tk.IntVar(value=3)
        self.num_threads   = tk.IntVar(value=min(4, _IM_NUM_CORES))

        self.face_scale     = tk.DoubleVar(value=1.15)
        self.face_neighbors = tk.IntVar(value=4)
        self.face_minsize   = tk.IntVar(value=40)
    ...
    def _build(self):
        ...
        # ── Cabeçalho ─────────────────────────────────────────────────────
        hdr = tk.Frame(body, bg=CARD, padx=14, pady=12)
        hdr.pack(fill="x", pady=(0, 10))
        tk.Label(hdr, text="🛡  InvisibleMark v4.1",
                 font=("Segoe UI", 14, "bold"), bg=CARD, fg=IM_HIGHLIGHT).pack(side="left")
        tk.Label(hdr, text="  Proteção adversarial imperceptível de faces em vídeo",
                 font=FONT_STATUS, bg=CARD, fg=MUTED).pack(side="left", padx=8)

        # ── Verificação opencv ────────────────────────────────────────────
        if not _OPENCV_OK:
            warn_card = tk.Frame(body, bg="#2a1a1a", padx=14, pady=10)
            warn_card.pack(fill="x", pady=(0, 10))
            tk.Label(warn_card,
                     text="⚠  opencv-python não encontrado. Execute:  pip install opencv-python",
                     font=FONT_BOLD, bg="#2a1a1a", fg=RED).pack(anchor="w")
            tk.Button(warn_card, text="⬇  Instalar opencv-python agora", font=FONT_LABEL,
                      bg=RED, fg="white", relief="flat", padx=10, pady=4, cursor="hand2",
                      command=self._install_opencv).pack(anchor="w", pady=(6, 0))

        # ── Layout 2 colunas ──────────────────────────────────────────────
        cols = tk.Frame(body, bg=DARK)
        cols.pack(fill="both", expand=True)
        cols.columnconfigure(0, weight=3)
        cols.columnconfigure(1, weight=2)

        left  = tk.Frame(cols, bg=DARK)
        left.grid(row=0, column=0, sticky="nsew", padx=(0, 10))
        right = tk.Frame(cols, bg=DARK)
        right.grid(row=0, column=1, sticky="nsew")

        self._panel_video(left)
        self._panel_protect_mode(left)
        self._panel_output(left)
        self._panel_techniques(right)
        self._panel_speed(right)
        self._panel_detector(right)
        self._panel_log(body)

    # ── Seção helpers ─────────────────────────────────────────────────────
    def _section(self, parent, title, color=IM_GREEN_OK):
    ...
    def _btn(self, parent, text, cmd, color=IM_BTN_BLUE,
             font=("Segoe UI", 9, "bold"), pady=4):
        b = tk.Button(parent, text=text, command=cmd, bg=color, fg="white",
                      relief="flat", cursor="hand2", font=font, padx=12, pady=pady,
                      activebackground=IM_BTN_HOVER, activeforeground="white", bd=0)
        b.bind("<Enter>", lambda e: b.config(bg=IM_BTN_HOVER))
        b.bind("<Leave>", lambda e: b.config(bg=color))
        return b

    # ── Painéis ───────────────────────────────────────────────────────────
    def _panel_video(self, parent):
        f = self._section(parent, "📁  Vídeo de Entrada")
        row = tk.Frame(f, bg=CARD)
        row.pack(fill="x", pady=4)
        ttk.Entry(row, textvariable=self.video_path, width=36).pack(
            side="left", fill="x", expand=True, ipady=4)
        self._btn(row, "Abrir", self._pick_video).pack(side="left", padx=(8, 0))

        self.lbl_info = tk.Label(f, text="", bg=CARD, fg=MUTED, font=FONT_SMALL)
        self.lbl_info.pack(anchor="w", pady=(2, 0))
        self.lbl_audio_info = tk.Label(f, text="", bg=CARD, fg=MUTED, font=FONT_SMALL)
        self.lbl_audio_info.pack(anchor="w")

    def _panel_protect_mode(self, parent):
        f = self._section(parent, "🎯  Modo de Proteção")

        tk.Label(f, text=(
            "faces_only → detecta e protege só as faces  [RECOMENDADO — mais rápido]\n"
            "full_frame → aplica em todo o frame  [mais lento, máxima cobertura]"
        ), bg=CARD, fg=MUTED, font=FONT_SMALL, justify="left").pack(anchor="w", pady=(0, 6))

        row = tk.Frame(f, bg=CARD)
        row.pack(fill="x")
        for val, txt in [("faces_only", "🎭 Apenas Faces"), ("full_frame", "🖼  Frame Inteiro")]:
            ttk.Radiobutton(row, text=txt, variable=self.protect_mode,
                            value=val).pack(side="left", padx=(0, 20))

        tk.Frame(f, height=1, bg=CARD2).pack(fill="x", pady=8)

        str_row = tk.Frame(f, bg=CARD)
        str_row.pack(fill="x")
        self.lbl_strength = tk.Label(
            str_row,
            text=f"Intensidade: {self.strength_val.get()}  (máx imperceptível: 6)",
            bg=CARD, fg=MUTED, font=FONT_STATUS)
        self.lbl_strength.pack(anchor="w")
        ttk.Scale(f, from_=1, to=12, variable=self.strength_val, orient="horizontal",
                  command=self._update_strength_label).pack(fill="x")

        tk.Label(f, text="⚠  Acima de 6: perturbação pode ser levemente visível",
                 bg=CARD, fg=IM_AMBER, font=FONT_SMALL).pack(anchor="w", pady=(2, 0))

    def _panel_techniques(self, parent):
        f = self._section(parent, "⚗️  Técnicas Adversariais", color=PURPLE)

        techniques = [
            (self.use_adv_noise, "🔬 Ruído Adaptativo JND",   "Abaixo do limiar de percepção humana"),
            (self.use_landmark,  "👁  Ataque a Landmarks",    "Gaussiana suavizada — sem bordas"),
            (self.use_freq,      "📡 Ataque DCT 8×8",         "Perturba coef. AC de frequência média"),
            (self.use_color,     "🎨 Micro-Shift HSV",        "Hue ≤2°, Sat ≤1.5% (imperceptível)"),
            (self.use_temporal,  "⏱  Ghost Temporal 4%",      "Quebra re-identificação temporal"),
            (self.use_pixel,     "🔀 Shuffle Mínimo 2px",     "Desativa por padrão (pode ser visível)"),
        ]

        for var, name, desc in techniques:
            row = tk.Frame(f, bg=CARD)
        ...
        tk.Frame(f, height=1, bg=CARD2).pack(fill="x", pady=(8, 4))

        audio_row = tk.Frame(f, bg=CARD)
        audio_row.pack(fill="x", pady=3)
        ttk.Checkbutton(audio_row, text="🔊 Preservar Áudio Original",
                        variable=self.keep_audio).pack(side="left")
        self.lbl_audio_status = tk.Label(
            audio_row, text="  ↳ Remux via ffmpeg (sem recodificação)",
            bg=CARD, fg=MUTED, font=FONT_SMALL)
        self.lbl_audio_status.pack(side="left")

        if not _FFMPEG_OK:
            tk.Label(f, text="⚠  ffmpeg não encontrado — áudio indisponível",
                     bg=CARD, fg=RED, font=FONT_SMALL).pack(anchor="w")
            self.keep_audio.set(False)

    def _panel_speed(self, parent):
        f = self._section(parent, "⚡  Velocidade & Detecção", color=CYAN)

        for lbl_text, var, lo, hi, nota in [
            ("Frame skip:", self.frame_skip, 1, 10,
             "  (2 = 2× mais rápido)"),
            ("Cache detecção:", self.face_cache_n, 1, 10,
             "  frames"),
            (f"Threads ({_IM_NUM_CORES} cores):", self.num_threads, 1, max(1, _IM_NUM_CORES), ""),
        ]:
            r = tk.Frame(f, bg=CARD)
            r.pack(fill="x", pady=3)
            tk.Label(r, text=lbl_text, bg=CARD, fg=MUTED,
                     font=FONT_STATUS, width=20, anchor="w").pack(side="left")
            ttk.Spinbox(r, from_=lo, to=hi, textvariable=var, width=4).pack(side="left")
            if nota:
                tk.Label(r, text=nota, bg=CARD, fg=MUTED, font=FONT_SMALL).pack(side="left")

        self.lbl_speed_est = tk.Label(f, text="", bg=CARD, fg=GREEN, font=FONT_SMALL)
        self.lbl_speed_est.pack(anchor="w", pady=(4, 0))

        tk.Frame(f, height=1, bg=CARD2).pack(fill="x", pady=(8, 4))
        tk.Label(f, text="🔍  Detector Haar", bg=CARD, fg=GOLD,
                 font=("Segoe UI", 9, "bold")).pack(anchor="w", pady=(0, 4))

        for lbl, var, lo, hi, inc, fmt in [
            ("Scale Factor:", self.face_scale, 1.05, 1.5, 0.05, "%.2f"),
            ("Min Neighbors:", self.face_neighbors, 1, 10, 1, "%d"),
            ("Min Size (px):", self.face_minsize, 20, 200, 5, "%d"),
        ]:
            r = tk.Frame(f, bg=CARD)
            r.pack(fill="x", pady=2)
            tk.Label(r, text=lbl, bg=CARD, fg=MUTED,
                     font=FONT_STATUS, width=14, anchor="w").pack(side="left")
            kw = dict(from_=lo, to=hi, textvariable=var, width=6)
            if fmt != "%d":
                kw["increment"] = inc
                kw["format"] = fmt
            ttk.Spinbox(r, **kw).pack(side="left")

        tk.Label(f, text=(
            "⚠  Scale ↑ = mais lento, melhor detecção\n"
            "   Neighbors ↓ = mais faces, mais falsos positivos"
        ), bg=CARD, fg=MUTED, font=FONT_SMALL, justify="left").pack(anchor="w", pady=(6, 0))

    def _panel_detector(self, parent):
        # já embutido em _panel_speed para economizar espaço
        pass

    def _panel_output(self, parent):
        f = self._section(parent, "💾  Arquivo de Saída")
        row = tk.Frame(f, bg=CARD)
        row.pack(fill="x", pady=4)
        ttk.Entry(row, textvariable=self.output_path, width=36).pack(
            side="left", fill="x", expand=True, ipady=4)
        self._btn(row, "Salvar", self._pick_output).pack(side="left", padx=(8, 0))

        tk.Frame(parent, height=1, bg=CARD2).pack(fill="x", pady=8)
        self.btn_run = self._btn(
            parent, "🛡  PROTEGER VÍDEO", self._run,
            color=IM_BTN_GREEN, font=("Segoe UI", 11, "bold"), pady=10)
        self.btn_run.pack(fill="x", ipady=6, pady=4)

    def _panel_log(self, parent):
        f = tk.Frame(parent, bg=DARK)
        f.pack(fill="x", pady=(6, 10))

        bars = tk.Frame(f, bg=DARK)
        bars.pack(fill="x")
        self.progress = ttk.Progressbar(bars, mode="determinate")
        self.progress.pack(fill="x", pady=(0, 2))

        self.lbl_fps = tk.Label(bars, text="", bg=DARK, fg=WARN,
                                font=FONT_STATUS, anchor="w")
        self.lbl_fps.pack(fill="x")

        log_frame = tk.Frame(f, bg=CARD2)
        log_frame.pack(fill="x")
        self.log_text = tk.Text(log_frame, height=5, bg=CARD2, fg=GREEN,
        ...
        self.lbl_status = tk.Label(f, text="Aguardando…", bg=DARK, fg=MUTED,
                                   font=FONT_STATUS, anchor="w")
        self.lbl_status.pack(fill="x", pady=(4, 0))

    # ── Callbacks ─────────────────────────────────────────────────────────
    def _update_strength_label(self, _=None):
        v = self.strength_val.get()
        warn = "" if v <= 6 else f"  ⚠ ({v-6} acima do ideal)"
        self.lbl_strength.config(
            text=f"Intensidade: {v}  (máx imperceptível: 6){warn}")

    def _pick_video(self):
        p = filedialog.askopenfilename(
            title="Selecionar Vídeo",
            filetypes=[("Vídeos", "*.mp4 *.avi *.mov *.mkv *.wmv"), ("Todos", "*.*")])
        if not p:
            return
        self.video_path.set(p)
        pp = Path(p)
        self.output_path.set(str(pp.with_stem(pp.stem + "_protected")))
        ...
        try:
            ...
            if cap.isOpened():
                ...
                self.lbl_info.config(
                    text=f"  {w}×{h} px  •  {fps:.1f} fps  •  {n} frames  •  {dur:.1f}s",
                    fg=GREEN)
                skip = self.frame_skip.get()
                threads = self.num_threads.get()
                self.lbl_speed_est.config(
                    text=f"Estimativa: ~{fps * skip * threads:.0f} fps de throughput")
        except Exception:
            pass

        if _FFMPEG_OK:
            has_audio = _im_video_has_audio(p)
            if has_audio:
                self.lbl_audio_info.config(
                    text="🔊 Áudio detectado — será preservado se ativo", fg=GREEN)
            else:
                self.lbl_audio_info.config(
                    text="🔇 Sem faixa de áudio neste vídeo", fg=MUTED)
    ...
    def _log(self, msg, color=None):
        def _do():
            ...
            self.lbl_status.config(text=msg, fg=color or GREEN)
    ...
    def _run(self):
        if self.running:
            return
        if not _OPENCV_OK:
            messagebox.showerror("opencv-python não instalado",
    ...
    def _process_thread(self):
        global _IM_FACE_CACHE_FRAMES, _IM_LAST_FACES
        import cv2
        import shutil
        ...
        try:
            ...
            strength = self.strength_val.get()
            preserve_audio = self.keep_audio.get() and _FFMPEG_OK

            _IM_FACE_CACHE_FRAMES = self.face_cache_n.get()

            settings = {
                "mode": mode, "strength": strength,
                "adv_noise":       self.use_adv_noise.get(),
                "freq_disruption": self.use_freq.get(),
                "temporal":        self.use_temporal.get(),
                "color_shift":     self.use_color.get(),
                "landmark_noise":  self.use_landmark.get(),
                "pixel_shuffle":   self.use_pixel.get(),
            }
            det_params = {
                "scale":     self.face_scale.get(),
                "neighbors": self.face_neighbors.get(),
                "min_size":  self.face_minsize.get(),
            }

            self._log("Inicializando detector facial (Haar + CLAHE)…")
            detector = _im_get_face_detector()
            ...
            fc_map = {".mp4": "mp4v", ".avi": "XVID", ".mov": "mp4v"}
            fourcc = cv2.VideoWriter_fourcc(*fc_map.get(ext, "mp4v"))

            if preserve_audio and _im_video_has_audio(video):
                tmp_dir = tempfile.mkdtemp()
                tmp_video = os.path.join(tmp_dir, "video_only" + ext)
                writer = cv2.VideoWriter(tmp_video, fourcc, fps, (fw, fh))
                self._log("🔊 Áudio será preservado — gravando em arquivo temporário…")
            else:
                tmp_dir   = None
                tmp_video = None
                writer    = cv2.VideoWriter(output, fourcc, fps, (fw, fh))
                if preserve_audio:
                    self._log("🔇 Vídeo sem áudio — ignorando preservação.", WARN)

            self._log(
                f"Vídeo: {fw}×{fh} | {total} frames | {fps:.1f}fps | "
                f"skip={skip} | mode={mode} | strength={strength}")
            ...
            for i in range(total):
                ...
                if skip == 1 or i % skip == 0:
                    faces = []
                    if mode == "faces_only":
                        faces = _im_detect_faces(
                            frame, detector,
                            scale=det_params["scale"],
                            neighbors=det_params["neighbors"],
                    ...
                    frame = _im_apply_pipeline(
                        frame, prev_frame, faces, settings, i)
                    prev_frame = frame
                ...
                if i % 30 == 0:
                    ...
                    eta = (total - i) / real_fps if real_fps > 0 else 0
                    self.after(0, lambda r=real_fps, e=eta, ft=faces_total:
                               self.lbl_fps.config(
                                   text=f"⚡ {r:.1f} fps  |  ETA: {e:.0f}s  |  Faces: {ft}"))

                if i % 50 == 0:
                    self._log(f"Frame {i+1}/{total} ({pct}%)  |  Faces: {faces_total}")
            ...
            if tmp_video is not None:
                self._log("🔊 Mesclando áudio original no vídeo processado…")
                self.after(0, lambda: self.progress.config(value=98))
                try:
                    _im_merge_audio(video, tmp_video, output)
                    self._log("✅ Áudio mesclado com sucesso (sem recodificação)")
                except Exception as audio_err:
                    self._log(f"⚠  Falha no merge de áudio: {audio_err}", IM_AMBER)
            ...
            self._log(
                f"✅ Concluído em {elapsed:.1f}s  |  {avg_fps:.1f} fps médio  |  {size_mb:.1f} MB")
            self.after(0, lambda: self.progress.config(value=100))

            audio_line = "✔ Áudio original preservado\n" if tmp_video else ""
            self.after(0, lambda: messagebox.showinfo("InvisibleMark — Concluído", (
                f"✔ {total} frames processados\n"
                f"✔ {frames_with_faces} frames com faces detectadas\n"
                f"✔ {faces_total} instâncias de faces protegidas\n"
                f"{audio_line}"
                f"✔ {avg_fps:.1f} fps médio  ({elapsed:.1f}s total)\n"
                f"✔ {size_mb:.1f} MB\n\n"
                f"Arquivo: {output}"
            )))

        except Exception as ex:
            self._log(f"❌ Erro: {ex}", RED)
            self.after(0, lambda: messagebox.showerror("Erro — InvisibleMark", str(ex)))
        finally:
            self.running = False
            self.after(0, lambda: self.btn_run.config(
                state="normal", text="🛡  PROTEGER VÍDEO"))

    def _install_opencv(self):
        def _do():
            ok, err = _pip_install("opencv-python")
            importlib.invalidate_caches()
            global _OPENCV_OK
            _OPENCV_OK = _check_opencv_silent()
            if _OPENCV_OK:
                ...
                self.after(0, lambda: messagebox.showerror(
                    "Falha na instalação",
                    f"Não foi possível instalar opencv-python.\n\n{err}"))
...
def _baixar_e_substituir(asset_url, destino, progresso_cb=None, log_cb=None):
    import urllib.request
    import shutil
    ...
    try:
        ...
        os.close(tmp_fd)
        req = urllib.request.Request(asset_url, headers={"User-Agent": f"AudioFix/{VERSAO_ATUAL}"})
        with urllib.request.urlopen(req, timeout=60) as resp, open(tmp_path, "wb") as f:
            ...
            while True:
                ...
                if total > 0 and progresso_cb:
                    progresso_cb(min(90, int(baixado / total * 85) + 5))
...
class UpdateDialog(tk.Toplevel):
    def __init__(self, parent, versao_nova, notas, asset_url, script_path):
    ...
    def _build(self):
        ...
        tk.Label(hdr, text=f"Versão atual: v{VERSAO_ATUAL}   →   Nova: v{self._versao_nova}",
                 font=FONT_LABEL, bg="#14141f", fg=TEXT).pack(pady=(4, 0))
        notes_frame = tk.Frame(self, bg=CARD, padx=14, pady=10)
        notes_frame.pack(fill="both", expand=True, padx=14, pady=(10, 6))
        tk.Label(notes_frame, text="📋  O que há de novo:", font=FONT_BOLD, bg=CARD, fg=MUTED, anchor="w").pack(anchor="w")
        notes_txt = tk.Text(notes_frame, height=8, font=FONT_MONO, bg="#111116", fg=TEXT,
                            relief="flat", state="normal", wrap="word", selectbackground=ACCENT)
        notes_txt.pack(fill="both", expand=True, pady=(4, 0))
        notes_txt.insert("1.0", self._notas.strip() or "Sem notas de release.")
        notes_txt.config(state="disabled")
        self._prog_frame = tk.Frame(self, bg=DARK, padx=14, pady=4)
        self._prog_frame.pack(fill="x")
        self._prog_bar = ttk.Progressbar(self._prog_frame, mode="determinate", maximum=100, length=530)
        self._prog_bar.pack(fill="x")
        self._prog_lbl = tk.Label(self._prog_frame, text="", font=FONT_STATUS, bg=DARK, fg=CYAN, anchor="w")
        self._prog_lbl.pack(anchor="w", pady=(2, 0))
        self._prog_frame.pack_forget()
        btn_row = tk.Frame(self, bg=DARK, pady=10); btn_row.pack(fill="x", padx=14)
        self._sim_btn = tk.Button(btn_row, text="✅  Sim, atualizar", font=FONT_BOLD,
                                   bg=GREEN, fg=DARK, activebackground=LIME,
                                   relief="flat", padx=16, pady=7, cursor="hand2",
                                   command=self._iniciar_atualizacao)
        self._sim_btn.pack(side="left")
        self._nao_btn = tk.Button(btn_row, text="✖  Continuar assim", font=FONT_LABEL,
                                   bg=CARD2, fg=MUTED, relief="flat", padx=12, pady=7,
    ...
    def _iniciar_atualizacao(self):
        ...
        def _prog(pct): self.after(0, lambda: self._prog_bar.config(value=pct))
        def _log(msg):  self.after(0, lambda: self._prog_lbl.config(text=msg))

        def worker():
            ok = _baixar_e_substituir(self._asset_url, self._script_path,
                                      progresso_cb=_prog, log_cb=_log)
            if ok:
                _log("🔄  Reiniciando…")
...
def verificar_atualizacao(parent_window, silencioso=True):
    def _worker():
        dados = _buscar_ultima_versao_github()
        if dados is None:
            if not silencioso:
                parent_window.after(0, lambda: messagebox.showwarning(
                    "Verificação", "Não foi possível verificar atualizações."))
            return
        ...
        if not _versao_maior(versao_remota, VERSAO_ATUAL):
            if not silencioso:
                parent_window.after(0, lambda: messagebox.showinfo(
                    "AudioFix Pro", f"Você está na versão mais recente (v{VERSAO_ATUAL}). ✅"))
            return
        ...
        for a in assets:
            if a.get("name", "").lower().endswith(".py"):
                ...
                break
        if not asset_url:
            script_name = os.path.basename(sys.argv[0] if sys.argv[0].endswith(".py") else __file__)
            for branch in ("main", "master"):
                candidate = (f"https://raw.githubusercontent.com/{GITHUB_REPO}"
                             f"/refs/heads/{branch}/{script_name}")
                try:
                    import urllib.request
                    req = urllib.request.Request(candidate, headers={"User-Agent": f"AudioFix/{VERSAO_ATUAL}"})
                    with urllib.request.urlopen(req, timeout=10) as resp:
        ...
        if not asset_url:
            def _sem_asset():
                ir = messagebox.askyesno("Atualização",
                    f"Nova versão v{versao_remota}!\nDeseja abrir a página de download?")
                if ir:
                    import webbrowser
        ...
        parent_window.after(0, lambda: UpdateDialog(
            parent_window, versao_nova=versao_remota, notas=notas,
            asset_url=asset_url, script_path=script_path))
...
TTS_RATE_PRESETS = {
    "Muito Lenta":0.78,"Lenta":0.88,"Normal":1.00,
    "Rápida":1.12,"Muito Rápida":1.25,"Ultra Rápida":1.45,
}
EDGE_TTS_RATE_MAP = {
    "Muito Lenta":"-30%","Lenta":"-15%","Normal":"+0%",
    "Rápida":"+15%","Muito Rápida":"+30%","Ultra Rápida":"+50%",
}
...
def _apply_ffmpeg_processing(input_path, output_wav, pitch_factor=1.0, rate_factor=1.0,
                              volume_db=0.0, eq_filter="", reverb_filter=None,
                              normalize=False, compressor=False,
                              comp_threshold=-18.0, comp_ratio=3.0,
                              comp_attack=20.0, comp_release=200.0,
                              deesser=False, stereo_width=1.0, pitch_semitones=0.0):
    original_sr = 44100
    total_semitones = 12 * math.log2(max(pitch_factor, 0.1)) + pitch_semitones
    ...
    if compressor:
        ...
        af_parts.append(f"acompressor=threshold={thresh_lin:.5f}:ratio={comp_ratio:.1f}"
                        f":attack={comp_attack:.0f}:release={comp_release:.0f}:makeup=1")
    if deesser:
        af_parts.append("equalizer=f=6500:width_type=o:width=1.5:g=-4,"
    ...
    if abs(volume_db) > 0.001 or volume_db <= -59.9:
        if volume_db <= -59.9:
            ...
            af_parts.append(f"volume={10**(volume_db/20.0):.6f}")
    if reverb_filter:
    ...
    cmd = ["ffmpeg", "-y", "-i", input_path, "-af", af_string, "-ar", "44100", "-ac", "2", output_wav]
...
def _generate_tts_gtts(text, voice_key, rate_preset, **proc_kwargs):
    ...
    tts = gTTS(text=text, lang=lang, tld=tld, slow=(rate < 0.90))
    tts.save(mp3_path)
...
def _generate_tts_edge(text, voice_key, rate_preset, **proc_kwargs):
    ...
    if not os.path.isfile(mp3_path) or os.path.getsize(mp3_path) < 100:
        raise RuntimeError("Edge TTS: arquivo gerado vazio. Verifique a conexão.")
...
def _generate_tts_elevenlabs(text, voice_key, api_key, stability=0.50, similarity=0.75,
                               style=0.0, use_speaker_boost=True, **proc_kwargs):
...
def generate_tts_wav(engine, text, voice_key, rate_preset="Normal",
    ...
    if engine == "gtts":
        return _generate_tts_gtts(text, voice_key, rate_preset, **proc_kw)
    elif engine == "edge":
        return _generate_tts_edge(text, voice_key, rate_preset, **proc_kw)
...
VARIATION_LEVELS = {
    "Normal":   {"label":"Normal",   "desc":"Mudanças sutis","color":"#4ade80","title_len":4,"artist_len":4,"album_len":3,"comment_len":6,"date_range":30,"track_range":(1,20),"encoder_var":False},
    "Moderada": {"label":"Moderada", "desc":"Alterações médias","color":"#fbbf24","title_len":6,"artist_len":5,"album_len":4,"comment_len":9,"date_range":365,"track_range":(1,50),"encoder_var":True},
    "Intensa":  {"label":"Intensa",  "desc":"Metadados bem distintos","color":"#f97316","title_len":8,"artist_len":7,"album_len":6,"comment_len":12,"date_range":1095,"track_range":(1,99),"encoder_var":True},
    "Brusca":   {"label":"Brusca",   "desc":"Completamente aleatórios","color":"#f87171","title_len":12,"artist_len":10,"album_len":9,"comment_len":18,"date_range":3650,"track_range":(1,999),"encoder_var":True},
}
...
def process_audio(input_wav, output_wav, noise_amplitudes=None,
    ...
    if copy_white_path and os.path.isfile(copy_white_path):
        ...
        cw_len=len(cw)
        if cw_len<n:
            repeats=int(math.ceil(n/cw_len)); cw=np.tile(cw,(repeats,1))
        cw=cw[:n]; left+=cw[:,0]*copy_white_gain; right+=cw[:,1]*copy_white_gain
...
class CollapsibleSection(tk.Frame):
    _HDR_BG="#111116"; _HDR_BG_HOV="#1a1a26"
    def __init__(self, parent, title, icon="", color=MUTED, default_open=True, badge_text="", **kwargs):
        super().__init__(parent, bg=DARK, **kwargs)
    ...
    def _build(self,title,icon):
        ...
        self._title_lbl=tk.Label(self._hdr,text=full_title,font=FONT_BOLD,bg=self._HDR_BG,fg=TEXT,pady=6)
        self._title_lbl.pack(side="left",fill="x",expand=True)
        if self._badge_text:
            tk.Label(self._hdr,text=self._badge_text,font=FONT_SMALL,bg=self._HDR_BG,fg=self._color,padx=6).pack(side="right")
...
class BatchQueueItem(tk.Frame):
    STATUS_PENDING="pending"; STATUS_RUNNING="running"; STATUS_DONE="done"
    STATUS_ERROR="error"; STATUS_CANCELLED="cancelled"
...
class BatchQueuePanel(tk.Frame):
    ...
    def _browse_folder(self):
        ...
        if added==0: messagebox.showinfo("Pasta vazia","Nenhum vídeo encontrado.")
    def _clear_queue(self):
        if any(i.status==BatchQueueItem.STATUS_RUNNING for i in self._items):
            messagebox.showwarning("Processamento em andamento","Aguarde terminar."); return
...
class TTSPanel(tk.Frame):
    _BG="#0a0a14"; _BG_HDR="#14141f"; _BG_HOV="#1c1c2e"; _BG_SEC="#12121c"
    ...
    def __init__(self,parent,on_wav_ready=None,bg=CARD,**kwargs):
        ...
        self.on_wav_ready=on_wav_ready; self._bg=bg
        self._last_wav=None; self._preview_proc=None
        self._engine=tk.StringVar(value="edge"); self._expanded=False
        self._build()

    def _build(self):
        bg=self._bg
        hdr=tk.Frame(self,bg=bg); hdr.pack(fill="x")
        self._toggle_btn=tk.Button(hdr,text="🎙  ▶  Gerar voz (TTS) para Copy White",
            font=FONT_BOLD,bg=self._BG_HDR,fg=PURPLE,relief="flat",padx=10,pady=6,
            cursor="hand2",anchor="w",command=self._toggle)
        ...
        tk.Label(status_frame,font=FONT_SMALL,bg=bg,text="ElevenLabs —",fg=MUTED).pack(side="left")
    ...
    def _build_engine_selector(self):
        ...
        for idx,(engine_key,label) in enumerate([
            ...
            lbl=tk.Label(inner,text=label,font=("Segoe UI",8,"bold"),bg=self._BG_SEC,fg=TEXT,justify="center")
        ...
        tk.Button(install_row,text="⬇  Instalar Edge TTS",font=FONT_SMALL,
                  bg="#0d1a14",fg=CYAN,relief="flat",padx=8,pady=2,cursor="hand2",
                  command=self._install_edge_tts).pack(side="left")
        tk.Button(install_row,text="⬇  Instalar gTTS",font=FONT_SMALL,
                  bg="#0d0d1a",fg=MUTED,relief="flat",padx=8,pady=2,cursor="hand2",
                  command=self._install_gtts).pack(side="left",padx=(4,0))
    ...
    def _build_text_area(self):
        # Template selector
        tmpl_row=tk.Frame(self._body,bg=self._BG); tmpl_row.pack(fill="x",pady=(0,4))
        tk.Label(tmpl_row,text="📋  Template:",font=FONT_BOLD,bg=self._BG,fg=MUTED).pack(side="left")
        ...
        tk.Button(tmpl_row,text="Aplicar",font=FONT_LABEL,bg=CARD2,fg=PURPLE,
                  relief="flat",padx=6,pady=1,cursor="hand2",
                  command=self._apply_template).pack(side="left",padx=(4,0))
        self._char_lbl=tk.Label(tmpl_row,text="0 chars",font=FONT_SMALL,bg=self._BG,fg=MUTED)
        self._char_lbl.pack(side="right")
        tk.Label(self._body,text="Texto para narrar:",font=FONT_BOLD,bg=self._BG,fg=MUTED).pack(anchor="w",pady=(0,3))
        self._txt=tk.Text(self._body,height=4,font=("Consolas",9),bg="#100d1e",fg=TEXT,
                          insertbackground=PURPLE,relief="flat",bd=1,wrap="word",
    ...
    def _build_voice_selector(self):
        ...
        self._voice_cb=ttk.Combobox(vc,textvariable=self._voice_var,state="readonly",width=36,font=FONT_LABEL)
        ...
        self._lang_cb=ttk.Combobox(lc,textvariable=self._lang_filter,state="readonly",width=16,font=FONT_LABEL)
        self._lang_cb.pack(fill="x",pady=(2,0))
        self._lang_cb.bind("<<ComboboxSelected>>",self._filter_voices)
        self._voice_badge=tk.Label(self._body,text="",font=FONT_SMALL,bg=self._BG,fg=ACC2,anchor="w")
    ...
    def _build_speed_row(self):
        ...
        rate_cb=ttk.Combobox(row,textvariable=self._rate_var,values=list(TTS_RATE_PRESETS.keys()),
                             state="readonly",width=14,font=FONT_LABEL)
        ...
        self._pitch_st_lbl=tk.Label(row,text="  0.0 st",font=("Segoe UI",9,"bold"),bg=self._BG,fg=PURPLE,width=8)
        self._pitch_st_lbl.pack(side="left")
        pitch_slider=ttk.Scale(row,from_=-12.0,to=12.0,variable=self._pitch_st_var,
                               orient="horizontal",command=self._on_pitch_slide,length=140)
        pitch_slider.pack(side="left",padx=(4,0))
        tk.Button(row,text="Reset",font=FONT_SMALL,bg=CARD2,fg=MUTED,relief="flat",padx=4,pady=1,
                  cursor="hand2",command=lambda:[self._pitch_st_var.set(0.0),
                  self._pitch_st_lbl.config(text="  0.0 st")]).pack(side="left",padx=(4,0))
    ...
    def _build_wav_processing(self):
        ...
        eq_cb=ttk.Combobox(eq_col,textvariable=self._eq_var,values=list(EQ_PRESETS.keys()),
                           state="readonly",width=22,font=FONT_LABEL)
        eq_cb.pack(fill="x",pady=(2,0))
        rev_col=tk.Frame(grid,bg=self._BG_SEC); rev_col.grid(row=0,column=1,sticky="ew",pady=(0,6))
        tk.Label(rev_col,text="🏠  Reverb:",font=FONT_BOLD,bg=self._BG_SEC,fg=MUTED).pack(anchor="w")
        self._reverb_var=tk.StringVar(value="Sem reverb")
        rev_cb=ttk.Combobox(rev_col,textvariable=self._reverb_var,values=list(REVERB_PRESETS.keys()),
                            state="readonly",width=22,font=FONT_LABEL)
        ...
        for var,label,color in [(self._deesser_var,"De-esser",TEAL),(self._norm_var,"Normalizar EBU R128",GREEN),(self._comp_var,"Compressor",ORANGE)]:
            tk.Checkbutton(eff_row,variable=var,text=label,font=FONT_LABEL,bg=self._BG_SEC,fg=color,
                           selectcolor="#0a0a14",activebackground=self._BG_SEC,relief="flat",cursor="hand2").pack(side="left",padx=(0,16))
        sw_row=tk.Frame(sec,bg=self._BG_SEC); sw_row.pack(fill="x",pady=(4,0))
        tk.Label(sw_row,text="↔  Estéreo:",font=FONT_BOLD,bg=self._BG_SEC,fg=MUTED).pack(side="left")
        self._sw_var=tk.DoubleVar(value=1.0)
        self._sw_lbl=tk.Label(sw_row,text="Normal (1.0×)",font=("Segoe UI",9,"bold"),bg=self._BG_SEC,fg=INDIGO,width=18)
        self._sw_lbl.pack(side="left",padx=(6,0))
        for label,val in [("Mono","0.0"),("Normal","1.0"),("Wide","1.5"),("Ultra","2.0")]:
            tk.Button(sw_row,text=label,font=FONT_SMALL,bg=CARD2,fg=MUTED,relief="flat",padx=4,pady=1,cursor="hand2",
                      command=lambda v=val:[self._sw_var.set(float(v)),self._update_sw_label()]).pack(side="left",padx=(2,0))
        sw_slider=ttk.Scale(sw_row,from_=0.0,to=2.0,variable=self._sw_var,orient="horizontal",
                            command=lambda v:self._update_sw_label(),length=120)
    ...
    def _build_volume_section(self):
        ...
        self._vol_lbl=tk.Label(vol_hdr,text="  0.0 dB",font=("Segoe UI",9,"bold"),bg=self._BG,fg=PURPLE,width=12)
        self._vol_lbl.pack(side="left")
        for label,val in [("-40",-40),("-20",-20),("-10",-10),("  0",0),("+10",10),("+20",20)]:
            tk.Button(vol_hdr,text=label,font=FONT_SMALL,bg=CARD2,fg=MUTED,relief="flat",
                      padx=4,pady=1,cursor="hand2",command=lambda v=val:self._set_volume(float(v))).pack(side="left",padx=(2,0))
    ...
    # ── Engine / Voice ────────────────────────────────────────────────────
    def _select_engine(self,key): self._engine.set(key); self._on_engine_change()
    ...
    def _on_voice_change(self,event=None):
        ...
        if engine=="gtts":
            ...
            self._voice_badge.config(text=f"  {g}  {cfg.get('lang','?')}-{cfg.get('tld','?')}  ·  pitch×{cfg.get('pitch',1.0):.2f}")
        elif engine=="edge":
            cfg=EDGE_TTS_VOICES.get(key,{}); g="♀" if cfg.get("gender")=="F" else "♂"
            self._voice_badge.config(text=f"  {g}  Neural · {cfg.get('lang','?')}  ·  {cfg.get('desc','')}  ·  {cfg.get('voice_id','?')}")
        else:
            cfg=ELEVENLABS_VOICES.get(key,{}); g="♀" if cfg.get("gender")=="F" else "♂"
            self._voice_badge.config(text=f"  {g}  ElevenLabs  ·  {cfg.get('desc','')}  ·  {cfg.get('voice_id','?')}")
    ...
    def _set_volume(self,val): self._vol_var.set(val); self._on_vol_slide(val)

    def _on_vol_slide(self,val=None):
        v=self._vol_var.get()
        if v<=-59.9: self._vol_lbl.config(text="  MUDO (−∞)",fg=RED)
        else:
            sign="+" if v>0 else ""; color=ORANGE if v>20 else (WARN if v>0 else (PURPLE if v>-20 else RED))
    ...
    def _install_edge_tts(self):
        ...
        def _do():
            ok=_ensure_edge_tts(); global _EDGE_TTS_OK; _EDGE_TTS_OK=ok
            if ok:
                ...
                self.after(0,lambda:self._install_lbl.config(text="❌ Falhou. pip install edge-tts",fg=RED))
    ...
    def _install_gtts(self):
        ...
        def _do():
            ...
            if _GTTS_OK:
                ...
                self.after(0,lambda:self._install_lbl.config(text="❌ Falhou. pip install gtts",fg=RED))
            self.after(3000,lambda:self._install_lbl.config(text=""))
        threading.Thread(target=_do,daemon=True).start()
    ...
    def _preview_wav(self):
        ...
        try:
            ...
            self._stop_btn.config(state="normal"); self._status_lbl.config(text="▶ Reproduzindo…",fg=TEAL)
            def _watch():
    ...
    def _run_tts(self):
        ...
        if engine=="gtts" and not _GTTS_OK:
            messagebox.showerror("gTTS não instalado","Clique em '⬇ Instalar gTTS'"); return
        if engine=="edge" and not _EDGE_TTS_OK:
            messagebox.showerror("Edge TTS não instalado","Clique em '⬇ Instalar Edge TTS'"); return
        if engine=="elevenlabs" and not _is_importable("requests"):
            messagebox.showerror("requests não instalado","pip install requests"); return
        text=self._txt.get("1.0","end").strip()
        if not text: messagebox.showwarning("Texto vazio","Digite o texto para narrar."); return
        ...
        compressor=self._comp_var.get(); deesser=self._deesser_var.get()
        ...
        def worker():
            try:
                wav=generate_tts_wav(engine=engine,text=text,voice_key=voice,
                    ...
                    deesser=deesser,stereo_width=sw,pitch_semitones=pitch_st,
                    el_api_key=el_key,el_stability=el_stab,el_similarity=el_sim,el_style=el_style)
                self._last_wav=wav
                engine_lbl=self.ENGINE_LABELS.get(engine,engine)
                self.after(0,lambda:self._wav_info.config(text=f"[{engine_lbl}]  {os.path.basename(wav)}\n  🗂  {wav}",fg=TEAL))
...
class TranscribeTab(tk.Frame):
    ...
    def _build(self):
        ...
        model_cb=ttk.Combobox(mod_frame,textvariable=self._model_var,values=WHISPER_MODELS,
        ...
        lang_cb=ttk.Combobox(lang_frame,textvariable=self._lang_var,values=list(WHISPER_LANGS.keys()),
        ...
        tk.Label(stat_frame,text="📦  Status Whisper",font=FONT_BOLD,bg=CARD,fg=MUTED).pack(anchor="w",pady=(0,3))
        wok=_WHISPER_OK
    ...
    def _browse_video(self):
        path=filedialog.askopenfilename(title="Selecionar vídeo",
            filetypes=[("Vídeos","*.mp4 *.mkv *.avi *.mov *.webm"),("Todos","*.*")])
    ...
    def _start_transcription(self):
        ...
        if not _WHISPER_OK:
            messagebox.showerror("Whisper não instalado","pip install faster-whisper"); return
        ...
        def worker():
            try:
                segs=transcribe_video(path,model_name=model,language=lang,progress_cb=prog_cb,log_cb=log_cb)
                self._segments=segs; self.after(0,lambda:self._render_result(segs))
            except Exception as e:
    ...
    def _render_result(self,segments):
        ...
        if not segments:
            tk.Label(self._res_frame,text="Nenhum segmento encontrado.",font=FONT_STATUS,bg=DARK,fg=MUTED,pady=20).pack(); return
        ...
        for i,seg in enumerate(segments):
            ...
            tk.Label(row,text=_format_timestamp(seg["start"]),font=("Consolas",8,"bold"),bg=row_bg,fg=CYAN,width=10,anchor="w").pack(side="left")
            tk.Label(row,text=_format_timestamp(seg["end"]),font=("Consolas",8),bg=row_bg,fg=MUTED,width=10,anchor="w").pack(side="left")
            tk.Label(row,text=seg["text"],font=("Segoe UI",9),bg=row_bg,fg=TEXT,anchor="w",wraplength=600,justify="left").pack(side="left",fill="x",expand=True)

    def _export(self,fmt):
        if not self._segments: messagebox.showwarning("Sem transcrição","Faça a transcrição primeiro."); return
        ext=f".{fmt}"
        path=filedialog.asksaveasfilename(title=f"Exportar {fmt.upper()}",defaultextension=ext,
            filetypes=[(fmt.upper(),f"*{ext}"),("Todos","*.*")])
        if not path: return
        try:
            with open(path,"w",encoding="utf-8") as f:
                if fmt=="txt":
                    for seg in self._segments: f.write(f"[{_format_timestamp(seg['start'])}]  {seg['text']}\n")
                elif fmt=="srt":
...
class PreviewTab(tk.Frame):
    ...
    def _build(self):
        ...
        tk.Entry(orig_row,textvariable=self._orig_path,font=FONT_MONO,bg=INPUT_BG,fg=INPUT_FG,
        ...
        tk.Entry(proc_row,textvariable=self._proc_path,font=FONT_MONO,bg=INPUT_BG,fg=INPUT_FG,
                 insertbackground="black",relief="flat",bd=1).pack(side="left",fill="x",expand=True)
    ...
    def _play_both(self):
        ...
        if not orig or not os.path.isfile(orig):
            messagebox.showwarning("Original","Selecione o vídeo original."); return
        if not proc or not os.path.isfile(proc):
            messagebox.showwarning("Processado","Selecione o vídeo processado."); return
...
class AudioFixApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title(f"AudioFix Pro + InvisibleMark — v{VERSAO_ATUAL}")
    ...
    def _build_ui(self):
        ...
        tk.Button(title_row,text="🔄  Verificar atualização",font=FONT_SMALL,bg="#1a1a2e",fg=ACC2,
                  relief="flat",padx=8,pady=3,cursor="hand2",
                  command=lambda:verificar_atualizacao(self,silencioso=False)).pack(side="left",padx=(16,0),pady=(6,0))
        tk.Label(hdr,
            text="Processamento em lote · Inversão de fase · Ruídos · Copy White · TTS Neural · Transcrição · 🛡 InvisibleMark",
        ...
        for text,color in [
            ...
            ("  |  ",MUTED),
            ("opencv ✔" if _OPENCV_OK else "opencv —",IM_HIGHLIGHT if _OPENCV_OK else MUTED),
            ("  |  ",MUTED),
    ...
    def _build_notebook(self):
        ...
        def switch_tab(name):
            ...
            for n,b in tab_btns.items():
                b.config(bg=ACCENT if n==name else "#111116",fg="white" if n==name else MUTED)

        def make_tab(name,label,icon,active_color=ACCENT):
            btn=tk.Button(tab_bar,text=f"{icon}  {label}",font=FONT_TAB,
                          bg="#111116",fg=MUTED,relief="flat",padx=14,pady=8,cursor="hand2",
        ...
        im_page      = make_tab("invisiblemark","InvisibleMark","🛡")
        ...
        # ── ABA INVISIBLEMARK ──────────────────────────────────────────────
        self.invisiblemark_tab = InvisibleMarkTab(im_page)
        self.invisiblemark_tab.pack(fill="both",expand=True)

        # Destaca a aba InvisibleMark com cor diferenciada
        tab_btns["invisiblemark"].config(fg=IM_HIGHLIGHT)
        def switch_tab_patched(name):
            switch_tab(name)
            tab_btns["invisiblemark"].config(
                bg=IM_BTN_BLUE if name=="invisiblemark" else "#111116",
                fg="white" if name=="invisiblemark" else IM_HIGHLIGHT)
        for n,b in tab_btns.items():
            b.config(command=lambda n=n:switch_tab_patched(n))

        switch_tab_patched("processar")
        self._switch_tab=switch_tab_patched
    ...
    def _build_process_page(self, parent):
        ...
        tk.Button(out_dir_row,text="Escolher pasta",font=FONT_LABEL,bg=ACCENT,fg="white",
                  relief="flat",padx=8,pady=2,cursor="hand2",command=self._browse_output_dir).pack(side="right")
        ...
        # ── Seções colapsáveis ────────────────────────────────────────────
        noise_section=CollapsibleSection(body,title="Mistura de ruídos coloridos",icon="🎨",color=PINK,default_open=True)
        ...
        cw_section=CollapsibleSection(body,title="Copy White — sobreposição de áudio externo",icon="📻",color=CYAN,default_open=True)
        ...
        dcb_section=CollapsibleSection(body,title="DCB — Dynamic Compression Boost",icon="🎚",color=GOLD,default_open=False)
        ...
        var_section=CollapsibleSection(body,title="Variações de metadados",icon="🎲",color=WARN,default_open=False)
        ...
        log_section=CollapsibleSection(body,title="Log de processamento",icon="📋",color=INDIGO,default_open=False)
    ...
    def _process_batch(self):
        ...
        if not items:
            messagebox.showinfo("Fila vazia","Adicione vídeos à fila."); return
        n=self.n_variants.get(); level=self.variation_level_selector.level
        noise=self._get_noise_amplitudes(); cw_path_val=None; cw_gain=self.cw_gain.get()
        if self.cw_enabled.get():
            cw_path_val=self.cw_path.get().strip()
    ...
    def _cancel_batch(self):
        ...
        if messagebox.askyesno("Cancelar lote?","Deseja cancelar?\nO vídeo atual será finalizado."):
...
class SplashScreen(tk.Tk):
    ...
    def _build(self):
        self.canvas=tk.Canvas(self,width=self.WIDTH,height=self.HEIGHT,bg="#0d0d0f",highlightthickness=0)
        self.canvas.pack(fill="both",expand=True)
        cx,cy=self.WIDTH//2,100; r=44
        self._ring1=self.canvas.create_arc(cx-r,cy-r,cx+r,cy+r,start=0,extent=300,outline="#7c6af7",width=4,style="arc")
        self._ring2=self.canvas.create_arc(cx-r+5,cy-r+5,cx+r-5,cy+r-5,start=60,extent=200,outline="#22d3ee",width=2,style="arc")
        self.canvas.create_text(cx,cy,text="🎛️",font=("Segoe UI Emoji",22),fill="#e8e8f0")
        self.canvas.create_text(self.WIDTH//2,155,text="AudioFix Pro + InvisibleMark",
                                font=("Segoe UI",15,"bold"),fill="#e8e8f0")
        self.canvas.create_text(self.WIDTH//2,175,
                                text=f"v{VERSAO_ATUAL}  ·  Proteção facial adversarial integrada",
        ...
        self._pct_txt=self.canvas.create_text(self.WIDTH//2,230,text="0%",font=("Segoe UI",8,"bold"),fill="#7c6af7")
        self._step_txt=self.canvas.create_text(self.WIDTH//2,246,text="Iniciando…",font=("Segoe UI",8),fill="#6b6b80")
        self.canvas.create_rectangle(0,0,self.WIDTH,3,fill="#7c6af7",outline="")
        self.canvas.create_rectangle(0,self.HEIGHT-3,self.WIDTH,self.HEIGHT,fill="#4d9eff",outline="")
...
def _run_loading(splash:SplashScreen):
    ...
    step(0,"Iniciando AudioFix Pro + InvisibleMark…",0.15)
    step(10,"Verificando Python 3.8+…",0.10)
    if sys.version_info<(3,8): sys.exit(1)
    step(20,"Carregando numpy / scipy…",0.10)
    _ensure_packages()
    step(35,"Verificando ffmpeg…",0.15)
    global _FFMPEG_OK,_GTTS_OK,_EDGE_TTS_OK,_WHISPER_OK,_REQUESTS_OK,_OPENCV_OK
    _FFMPEG_OK=_check_ffmpeg()
    step(45,"Verificando gTTS…",0.10)
    _GTTS_OK=_check_gtts_silent()
    step(55,"Verificando Edge TTS…",0.10)
    _EDGE_TTS_OK=_check_edge_tts_silent()
    step(65,"Verificando Whisper…",0.10)
    _WHISPER_OK=_check_whisper_silent()
    step(72,"Verificando requests…",0.05)
    _REQUESTS_OK=_check_requests_silent()
    step(80,"Verificando opencv (InvisibleMark)…",0.10)
    _OPENCV_OK=_check_opencv_silent()
    step(90,"Carregando módulos de áudio…",0.10)
    try:
        import numpy; from scipy.io import wavfile
    except: pass
    step(96,"Construindo interface…",0.20)
